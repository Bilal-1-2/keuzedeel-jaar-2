...
function formatNumber(n, dec = 2) { return Math.round(n * Math.pow(10, dec)) / Math.pow(10, dec); }
...
function update() {
  ...
  if (s) {
    ...
    const activeFrameIndex = s.currentFrame != null ? s.currentFrame : (s.fireFrameIndex || 0);
    const totalFrames = s.totalFrames || (s.activeFrames && s.activeFrames.length) || 0;
    ...
    if (s.bullets && s.bullets.length > 0) {
      ...
      if (s.bullets.length > 4) html += `<div>... +${s.bullets.length-4} more</div>`;
    }
    // Show firing/reload/melee frame indices if present
    html += '<div style="margin-top:4px">';
    if (s.fireFrameIndex != null) html += `fireFrame:${s.fireFrameIndex} `;
    if (s.reloadFrameIndex != null) html += `reloadFrame:${s.reloadFrameIndex} `;
